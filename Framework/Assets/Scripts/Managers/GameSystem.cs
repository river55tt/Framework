...
public class GameSystem : NetworkBehaviour
{
   ...
   //OnClient
        ...
        List<int> snappedfrom, List<int> snappedto, List<int> snapobjectIDs, List<int> anchorobjectIDs, int entityID, bool sync, out int serverID)
    {
        ...
        if(chunkID == -1 && !anchorIDs.ContainsKey(-1))
        {
            GameObject placingblock = Instantiate(terrainlist[0], Vector3.zero, Quaternion.identity); //place tile
            EntityProperties blockproperties = placingblock.GetComponent<EntityProperties>();
            blockproperties.entityID = -9999;
            GameSystem.GameManager.entityIDs.Add(-9999, blockproperties);
            AnchorSystem.AnchorManager.CreateObject(blockproperties); //connect to adjacent terrain
            blockproperties.anchordata.anchor = true;
            blockproperties.anchordata.chunk.physicsproperties.isKinematic = true;
            GameSystem.GameManager.anchorIDs.Add(-1, GameSystem.GameManager.entityIDs[-9999].anchordata.chunk);
            GameSystem.GameManager.anchorIDs[-1].anchorID = -1;
        }
    ...
    IEnumerator InitializeMatch()
    {
        ...
        if (SaveSystem.SaveManager.loadingdata)
        {
            SaveSystem.SaveManager.LoadData();
            TerrainSystem.TerrainManager.generatingterrain = true;
        }
        else
        {
    ...
    public void UpdateChunkMesh(Transform meshcombine)
    {
        ...
        for (int i = 1; i < meshFilters.Length; i++)
        {
            ...
            if (testmatrix)
            {
                combine[i].transform = meshcombine.worldToLocalMatrix * meshFilters[i].transform.localToWorldMatrix;
            }
            combinelist.Add(combine[i]);
            //meshcombine.worldToLocalMatrix* meshFilters[i].transform.localToWorldMatrix;

            meshFilters[i].gameObject.GetComponent<MeshRenderer>().enabled = false;
        }
        combine = combinelist.ToArray();
