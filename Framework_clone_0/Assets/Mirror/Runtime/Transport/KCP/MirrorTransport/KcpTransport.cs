...
// OnGUI allocates even if it does nothing. avoid in release.
        ...
        {
            long bufferlong = server.connections.Values.Sum(conn => conn.SendBufferCount);
            //Debug.Log(server.connections.Values.Sum(conn => conn.SendBufferCount));
            //if (bufferlong.HasValue)
            //{
                return Convert.ToInt32(bufferlong);
            //}
            return 0;
        }
